...
\section{Estimation of $\sigma^2_i$}
  ...
  \end{itemize}
  \end{center}
In this case instead we consider only one node, since we are interest to estimate $\sigma^2_i$.
So we have $m$ samples of node $i$ $$\upsilon_{i,1},...,\upsilon_{i,m}$$ each of them follows the distribution
\begin{equation}
    X_i\sim \mathcal{N}(\mu_i,\sigma^2_i)
    \label{eq:3}
\end{equation}
