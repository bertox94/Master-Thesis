...
\end{align}
The precision of this estimation can be further improved by imposing the constraint \eqref{eq:10} on the minimization. This condition is ignored if we use LLS, but it can be successfully employed with the method of Lagrange multipliers \cite{1275684,Lopez1994, pasa}.
...
\end{bmatrix}$$
where the $d_i$ are computed with the \eqref{eq:17}.\\\\
...
Below it is presented a summary of the proposed algorithms
\begin{center}
    \begin{tabular}{ |c|c|c|c| } 
    \hline
    \textbf{Approach} & \textbf{Ref} & \textbf{Pros} & \textbf{Cons}\\
    \hline
    Trilateration&\cite{rzk}&&\\
    \hline
    Min-Max&\cite{inproceedings}&\shortstack{a \\ bb \\ c}&\\
    \hline
    Quadratic linearization&\cite{rzk}&&\\
    \hline
    Lines intersection&\cite{rzk}&&\\
    \hline
    Linear estimation&\cite{rzk}&&\\
    \hline
    Weighted linear estimation&\cite{rzk}&&\\
    \hline
    Multilateration&\cite{rzk}&&\\
    \hline
    Maximum Likelihood&\cite{rzk}&&\\
    \hline
    Centroid algorithm&\cite{878533}&&\\
    \hline
    DV-Hop algorithm&\cite{965964}&&\\
    \hline
    Fingerprinting&\cite{YIU2017235}&&\\
    \hline
    \end{tabular}
\end{center}
...
\hline
Use Case Navn:          & Opret Server & add\\
\hline
Scenarie:               & At oprette en server med bestemte regler som tillader folk at spille sammen. More Text more text More Text &add\\
\hline
\end{tabularx}
